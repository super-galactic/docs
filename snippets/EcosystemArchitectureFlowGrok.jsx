export const SuperGalacticArchitectureFlow = () => {
  ...
  const flows = React.useMemo(
    () => ({
      ...
      spend: {
        ...
        steps: [
          { id: "gc_progression", caption: "Progression and upgrades executed in game" },
          { id: "edge_gc_to_hub_spend", caption: "Hub asset manager updates via sync webhook" },
          { id: "edge_hub_to_chain", caption: "Burn and treasury settlement executes on chain" },
  ...
  const activeStep = flows[activeFlow].steps[step] ? flows[activeFlow].steps[step] : null;
  const activeStepId = activeStep ? activeStep.id : null;
  const caption = activeStep ? activeStep.caption : "";
  ...
  const layout = React.useMemo(() => {
    if (mode === "mobile") {
      ...
      const edge = {
        gc_to_hub: `M ${x0 + w / 2} ${yGame + h} C ${x0 + w / 2} ${yGame + h + 32}, ${x0 + w / 2} ${yHub - 32}, ${x0 + w / 2} ${yHub}`,
        hub_to_chain: `M ${x0 + w / 2} ${yHub + h} C ${x0 + w / 2} ${yHub + h + 32}, ${x0 + w / 2} ${yChain - 32}, ${x0 + w / 2} ${yChain}`,
        chain_to_hub: `M ${x0 + w / 2 + 78} ${yChain} C ${x0 + w / 2 + 78} ${yChain - 32}, ${x0 + w / 2 + 78} ${yHub + h + 32}, ${x0 + w / 2 + 78} ${yHub + h}`,
        hub_to_gc: `M ${x0 + w / 2 + 78} ${yHub} C ${x0 + w / 2 + 78} ${yHub - 32}, ${x0 + w / 2 + 78} ${yGame + h + 32}, ${x0 + w / 2 + 78} ${yGame + h}`,
      };
    ...
    const midY = y + 270;
    const edge = {
      gc_to_hub: `M ${xGame + cardW} ${midY} C ${xGame + cardW + 62} ${midY - 40}, ${xHub - 62} ${midY - 40}, ${xHub} ${midY}`,
      hub_to_chain: `M ${xHub + cardW} ${midY} C ${xHub + cardW + 62} ${midY - 40}, ${xChain - 62} ${midY - 40}, ${xChain} ${midY}`,
      chain_to_hub: `M ${xChain} ${midY + 32} C ${xChain - 62} ${midY + 70}, ${xHub + cardW + 62} ${midY + 70}, ${xHub + cardW} ${midY + 32}`,
      hub_to_gc: `M ${xHub} ${midY + 32} C ${xHub - 62} ${midY + 70}, ${xGame + cardW + 62} ${midY + 70}, ${xGame + cardW} ${midY + 32}`,
    };
  ...
  const activeEdgeKey = React.useMemo(() => {
    ...
    if (activeStepId === "edge_gc_to_hub_spend") return "gc_to_hub";
  ...
  const isMicroActive = (id) => {
    ...
    // Spend special: step 2 should show hub asset manager update, triggered by progression update
    if (activeFlow === "spend" && activeStepId === "edge_gc_to_hub_spend") {
      if (id === "gc_progression") return true;
      if (id === "hub_asset") return true;
      return false;
    }

    // Spend special: step 4 is a return sync, show hub asset plus game progression as the visible state touch points
    if (activeFlow === "spend" && activeStepId === "edge_chain_to_hub_then_hub_to_gc") {
      if (id === "hub_asset") return true;
      if (id === "gc_progression") return true;
      return false;
    }
  ...
  const isContainerActive = (key) => {
    ...
    if (key === "hub" && (activeStepId === "edge_gc_to_hub" || activeStepId === "edge_chain_to_hub" || activeStepId === "edge_gc_to_hub_spend")) return true;
  ...
  const iconSvg = (name, active) => {
    ...
    const fill = "none";
    const common = { stroke, fill, strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round" };

    if (name === "mission") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M12 2l3 7 7 3-7 3-3 7-3-7-7-3 7-3 3-7z" /></svg>;
    if (name === "upgrade") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M12 19V5" /><path {...common} d="M6 11l6-6 6 6" /><path {...common} d="M5 21h14" /></svg>;
    if (name === "reward") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M7 8h10v12H7z" /><path {...common} d="M9 8V6a3 3 0 0 1 6 0v2" /><path {...common} d="M12 12v4" /></svg>;
    if (name === "cache") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M4 7h16" /><path {...common} d="M6 7v12" /><path {...common} d="M18 7v12" /><path {...common} d="M4 19h16" /></svg>;
    if (name === "asset") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M7 7h10v10H7z" /><path {...common} d="M9 5h6" /><path {...common} d="M9 19h6" /></svg>;
    if (name === "balance") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M4 7h16" /><path {...common} d="M6 7v12h12V7" /><path {...common} d="M8 11h8" /><path {...common} d="M8 15h6" /></svg>;
    if (name === "claim") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M12 3v10" /><path {...common} d="M8 9l4 4 4-4" /><path {...common} d="M5 21h14" /></svg>;
    if (name === "nft") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M7 7h10v10H7z" /><path {...common} d="M9 9h6v6H9z" /></svg>;
    if (name === "sync") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M7 7h6" /><path {...common} d="M7 7l2-2" /><path {...common} d="M7 7l2 2" /><path {...common} d="M17 17h-6" /><path {...common} d="M17 17l-2-2" /><path {...common} d="M17 17l-2 2" /></svg>;
    if (name === "token") return <svg width="16" height="16" viewBox="0 0 24 24"><circle {...common} cx="12" cy="12" r="7" /><path {...common} d="M9 12h6" /></svg>;
    if (name === "burn") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M12 22c4 0 7-3 7-7 0-4-3-7-7-11C8 8 5 11 5 15c0 4 3 7 7 7z" /></svg>;
    if (name === "treasury") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M6 9h12v12H6z" /><path {...common} d="M8 9V7a4 4 0 0 1 8 0v2" /></svg>;
    if (name === "verify") return <svg width="16" height="16" viewBox="0 0 24 24"><path {...common} d="M20 7l-9 10-4-4" /><path {...common} d="M12 2l8 4v6c0 5-3 9-8 10C7 21 4 17 4 12V6l8-4z" opacity="0.55" /></svg>;

    return <svg width="16" height="16" viewBox="0 0 24 24"><circle {...common} cx="12" cy="12" r="6" /></svg>;
  };
  ...
  const Container = ({ k, c }) => {
    ...
    return (
      <g>
        ...
        <text x={c.x + 46} y={c.y + 32} className="cardTitle">{c.title}</text>
        <text x={c.x + 46} y={c.y + 54} className="cardSub">{c.sub}</text>
        <line x1={c.x + 18} y1={c.y + 72} x2={c.x + c.w - 18} y2={c.y + 72} className="divider" />
        {(layout.micro[k] || []).map((t) => <MicroTile key={t.id} t={t} active={isMicroActive(t.id)} />)}
        {activeStepId === "hub_balance_claimable" && k === "hub" ? (
          <g>
            <rect x={c.x + c.w - 126} y={c.y + 18} width="108" height="22" rx="11" ry="11" className="badge" />
            <text x={c.x + c.w - 72} y={c.y + 34} textAnchor="middle" className="badgeText">Claimable</text>
          </g>
        ) : null}
      </g>
    );
  ...
  const tabStyleActive = {
    ...
    boxShadow: "0 0 0 1px rgba(124, 58, 237, 0.10), 0 10px 30px rgba(124, 58, 237, 0.12)",
  };
  ...
  return (
    <div className="wrap">
      <div className="topbar">
        ...
        <div className="rightControls">
          ...
          <div className="actions">
            <button type="button" style={actionBtn} onClick={startFromBeginning}>Play</button>
            <button type="button" style={actionBtn} onClick={reset}>Reset</button>
            <button
              type="button"
              style={actionBtn}
              onClick={() => {
      ...
      <div className="caption">
        ...
        <span className="captionRight">Step {Math.min(step + 1, totalSteps)} of {totalSteps}</span>
      </div>
      ...
      <div className="stage">
        <svg viewBox={`0 0 ${layout.W} ${layout.H}`} width="100%" height="100%" className="svg" preserveAspectRatio="xMidYMid meet">
          ...
          {mode === "desktop" ? (
            <g>
              <text x={layout.containers.game.x + layout.containers.game.w / 2} y="140" textAnchor="middle" className="colHeader">Gameplay layer</text>
              <text x={layout.containers.hub.x + layout.containers.hub.w / 2} y="140" textAnchor="middle" className="colHeader">Application layer</text>
              <text x={layout.containers.chain.x + layout.containers.chain.w / 2} y="140" textAnchor="middle" className="colHeader">On chain settlement</text>
            </g>
          ) : (
            <g>
              <text x={layout.W / 2} y="135" textAnchor="middle" className="colHeader">System map</text>
            </g>
          )}
          ...
          <g>
            <rect
              x={mode === "mobile" ? 60 : 46}
              y={mode === "mobile" ? layout.H - 116 : layout.H - 92}
              ...
              className="footer"
            />
            <text x={mode === "mobile" ? 84 : 68} y={mode === "mobile" ? layout.H - 88 : layout.H - 64} className="footerTitle">Key principles</text>
            <text x={mode === "mobile" ? 84 : 68} y={mode === "mobile" ? layout.H - 66 : layout.H - 42} className="footerText">
      ...
      <style jsx>{`
        ...
        .stage {
          ...
          background: radial-gradient(1100px 560px at 50% 20%, rgba(124, 58, 237, 0.14), rgba(2, 6, 23, 0.08));
        }
        ...
        @keyframes routePulse {
          0% { opacity: 0.70; stroke-width: 3.2; }
          50% { opacity: 1; stroke-width: 4.2; }
          100% { opacity: 0.70; stroke-width: 3.2; }
        }
        ...
        @media (prefers-color-scheme: light) {
          ...
          .h2 { color: rgba(0, 0, 0, 0.62); }
          .caption {
            border: 1px solid rgba(0, 0, 0, 0.10);
            background: rgba(0, 0, 0, 0.03);
          }
          .captionLabel { color: rgba(0, 0, 0, 0.68); }
          .captionText { color: rgba(0, 0, 0, 0.84); }
          .captionRight { color: rgba(0, 0, 0, 0.58); }
        }
      `}</style>
    </div>
  );
